...
$total = 0; $colis=0;
foreach ($ok as $key) {$total +=$key->total;$colis +=1;}
	...
	$pdf-> setXY($X+140,$Y+20);
		...
		$pdf->SetFont('Arial','i','10');
			$pdf-> Cell(50,5,date( 'j M Y H:i', strtotime($key->date)),'0','0','L');

		$pdf-> setXY($X+140,$Y+14);
			$pdf-> Cell(50,5,'Passer par : '.substr($key->vendeur??"pharmacie", 0,10),'0','0','L');

		$pdf-> setXY($X+140,$Y+22);
		...
		$pdf->SetFont('Arial','b',10);
			...
			$pdf-> Cell(109,7,'DESIGNATION','1','0','L','green');
			$pdf-> Cell(23,7,'P.UNIT','1','0','L','green');
			$pdf-> Cell(15,7,'QTE','1','0','L','green');
			$pdf-> Cell(15,7,'STOCK','1','0','L','green');
			...
			foreach ($ok as $key) {
				...
				$pdf-> Cell(109,7,$nom ,'BL','0','l');
				$pdf-> Cell(23,7, number_format($key->prix,0,' ',' '),'B','0','l');
				$pdf-> Cell(15,7, $key->qte ,'B','0','l');
				$pdf-> Cell(15,7, substr($key->en_stock??"Ras", 0,10) ,'B','0','l');
